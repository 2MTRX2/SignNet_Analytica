...
def test_name_property():
    # ARRANGE
    kb_bloch = KbCentralityBloch()
    
    # ACT & ASSERT
    assert kb_bloch.name == "K-B (Bloch, t=1 with d=t)"

# =====================================================================
# 2. COMPUTATION TESTS (compute)
# =====================================================================

@patch.object(KbCentralityBloch, "_prepare_core_system")
@patch.object(KbCentralityBloch, "_to_dataframe")
def test_compute_success(mock_to_dataframe, mock_prepare_system):
    ...
    expected_scores = mock_delta * (mock_A @ mock_b_scores)
    ...
    # ACT
    result_df = kb_bloch.compute(mock_network)

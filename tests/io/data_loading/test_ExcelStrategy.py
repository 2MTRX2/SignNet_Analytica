...
@pytest.fixture
def excel_strategy(mock_representation):
    return ExcelStrategy(representation=mock_representation)

# =====================================================================
# 2. READING
# =====================================================================

def test_read_raw_loads_and_caches_dataframe(excel_strategy):
    # ARRANGE
    fake_file = "dummy_network.xlsx"
    fake_df = pd.DataFrame({"source": [1, 2], "target": [3, 4]})

    with patch("pandas.read_excel", return_value=fake_df) as mock_read_excel:
        # 1. test reading
        result_1 = excel_strategy.read_raw(fake_file)
...
def test_load_delegates_to_representation_handler(excel_strategy, mock_representation):
    # ARRANGE
    fake_file = "dummy_network.xlsx"
    ...
    with patch("pandas.read_excel", return_value=fake_df):
        # ACT
        result = excel_strategy.load(fake_file)
...
def test_is_subclass_of_data_loading_strategy(excel_strategy):
    from signnet.io.data_loading.DataLoadingStrategy import DataLoadingStrategy
    assert isinstance(excel_strategy, DataLoadingStrategy)
